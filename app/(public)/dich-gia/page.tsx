...
import {
  Globe2, BarChart3, PlusCircle, Coins, ChevronRight,
  Eye, MessageSquare, FileText, BookMarked, BookOpen, ExternalLink,
  AlertCircle, Clock
} from 'lucide-react'
...
interface Story {
  ...
  viewCount: number; totalEarnings: number; sourceAuthor: string | null; sourceLanguage: string | null
  _count: { chapters: number; comments: number; bookmarks: number }
  pendingChapters?: number; rejectedChapters?: number
}

const STATUS_BADGE: Record<string, { label: string; cls: string }> = {
  ONGOING:   { label: 'Đang dịch',  cls: 'bg-green-500/10 text-green-600' },
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-black flex items-center gap-2">
            <Globe2 className="w-7 h-7 text-primary" /> Dashboard Dịch giả
          </h1>
        ...
        <Link href="/dich-gia/truyen/them"
          ...
          <PlusCircle className="w-4 h-4" /> Đăng bản dịch mới
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Bản dịch',    value: stories.length,               icon: Globe2,     color: 'text-primary' },
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <Link href="/dich-gia/thu-nhap" className="flex items-center gap-3 p-4 rounded-2xl border border-border hover:border-primary/30 hover:bg-primary/5 transition-all group">
        ...
        <Link href="/dich-gia/truyen" className="flex items-center gap-3 p-4 rounded-2xl border border-border hover:border-primary/30 hover:bg-primary/5 transition-all group">
          <FileText className="w-5 h-5 text-blue-500" />
          <div><p className="font-semibold text-sm">Quản lý bản dịch</p><p className="text-xs text-muted-foreground">{stories.length} truyện</p></div>
          <ChevronRight className="w-4 h-4 ml-auto text-muted-foreground group-hover:translate-x-1 transition-transform" />
        </Link>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <h2 className="font-bold text-lg">Bản dịch của tôi</h2>
          <div className="flex items-center gap-2 flex-wrap">
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as any)}
              ...
              <option value="ONGOING">Đang dịch</option>
            ...
            <Link href="/dich-gia/truyen" className="text-sm text-primary hover:underline">Xem tất cả →</Link>
          </div>
        </div>

        {displayed.length === 0 ? (
          <div className="py-16 text-center rounded-2xl border border-dashed border-border">
            <Globe2 className="w-12 h-12 mx-auto mb-3 text-muted-foreground/30" />
            <p className="text-muted-foreground">{stories.length === 0 ? 'Bạn chưa có bản dịch nào.' : 'Không có truyện nào phù hợp bộ lọc.'}</p>
            {stories.length === 0 && (
              <Link href="/dich-gia/truyen/them" className="mt-4 inline-flex items-center gap-2 px-5 py-2 rounded-xl gradient-primary text-white text-sm font-semibold hover:opacity-90">
                <PlusCircle className="w-4 h-4" /> Đăng bản dịch đầu tiên
              </Link>
            )}
          </div>
        ) : (
              ...
              return (
                <div key={story.id} className={`flex items-center gap-4 p-4 hover:bg-muted/40 transition-colors ${hasIssue ? 'border-l-2 border-red-500' : hasPending ? 'border-l-2 border-amber-400' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <Link href={`/dich-gia/truyen/${story.id}`} className="font-bold text-sm hover:text-primary transition-colors line-clamp-1">{story.title}</Link>
                    ...
                    {story.sourceAuthor && (
                      <p className="text-xs text-muted-foreground mt-0.5">Tác giả gốc: {story.sourceAuthor} {story.sourceLanguage && `(${story.sourceLanguage})`}</p>
                    )}
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    ...
                    <Link href={`/dich-gia/truyen/${story.id}/chuong`}
                      className="px-3 py-1.5 rounded-lg border border-border text-xs font-medium hover:bg-muted transition-colors">Chương</Link>
                    <Link href={`/dich-gia/truyen/${story.id}`}
                      className="px-3 py-1.5 rounded-lg gradient-primary text-white text-xs font-medium hover:opacity-90 transition-opacity">Sửa</Link>
                  </div>
                </div>
              )
            })}
